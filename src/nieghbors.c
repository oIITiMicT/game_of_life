...
int calc_nieghbors(Map *cur_field, int i, int j) {
    ...
    if (i < cur_field->length - 1) {
        if (j > 0 && cur_field->arr[i + 1][j - 1].live) cnt++;
        if (j < cur_field->width - 1 && cur_field->arr[i + 1][j + 1].live) cnt++;
        if (cur_field->arr[i + 1][j].live) cnt++;
    }
    if (j > 0 && cur_field->arr[i][j - 1].live) cnt++;
    if (j < cur_field->length - 1 && cur_field->arr[i][j + 1].live) cnt++;
    return cnt;
}